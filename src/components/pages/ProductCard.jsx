...
export default function ProductCard() {
  ...
  let data = products.filter((product) => product.id === test.id);
  const [x, setX] = useState(data[0].stock);
  console.log("productCatrd data", data[0].stock);
  // function addBasket(){
  //   let basketItem  = [];
    ...
    <div className="card-container">
        ...
        <div className="information">
          <section className="PCardTop">
            <h1>{data[0].name}</h1>
            <h4>{Math.floor(data[0].price - (data[0].price * data[0].sale) / 100)}$</h4>
            <p>Availability: {data[0].stock}</p>
            
            <p>Hurry up! only {data[0].stock} product left in stock!</p>
          </section>
